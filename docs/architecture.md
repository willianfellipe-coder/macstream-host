...
Secure host lock is coordinated in `AppState` and rendered by the app process, not by the video engine. `SecureLockReadiness` separates product decisions before locking: ready, missing MacStream password, no safe display during capture risk, or lockout. During capture risk, every physical display gets a full-screen local shield; the streamed display's shield is visual-only, non-key, mouse-transparent, and marked `sharingType = .none`, while the password panel is rendered only on a non-streamed display. If no non-streamed display exists, lock is refused rather than risking a password prompt on the Moonlight capture target.
